...
async function refreshAccessToken(token: JWT): Promise<JWT> {
  try {
    ...
    if (!res.ok || refreshed.error) {
      ...
      // Return existing token with error flag — caller should force re-login
    ...
    return {
      ...
      // New expiry = now + expires_in (seconds). Store as unix ms.
      accessTokenExpires:
...
export const authOptions: NextAuthOptions = {
  providers: [
    GoogleProvider({
      ...
      authorization: {
        params: {
          // Request drive.file so we can create spreadsheets in the user's Drive
  ...
  callbacks: {
    async jwt({ token, user, account }) {
      // ── First sign-in: capture all OAuth tokens ──────────────────────────
      if (account && user?.email) {
        token.email = user.email;
        token.accessToken = account.access_token ?? "";
        token.refreshToken = account.refresh_token ?? "";
        // Google returns expires_in (seconds) on first sign-in
      ...
      // Token is expired (or within the buffer) — refresh it
    ...
    async session({ session, token }) {
      ...
      // Surface refresh errors to the client so UI can prompt re-login
